<script>
    ...
    import Interreg from '../assets/intereg.png'
    export let descList = []
    ...
    function getLink(item) {
        ...
        // https://explore.openaire.eu/search/find/projects?fv0=e-shape%20igme&f0=q&page=1
        return logo
    }
    let isActive = false;
    function toggle() {
    ...
    {#if descList.length > 0 }     
      {#each descList as item}
        <div class="py-3">
            ...
            <dd class={`${isActive ? 'line-clamp-none cursor-auto' : 'line-clamp-4 cursor-grab'} mb-1 text-gray-500 md:text-sm dark:text-gray-400`} on:click={toggle}>{item.desc}</dd>
        </div>
      {/each}
    {/if}
</dl>
...
<!-- <div class="fixed bottom-0 left-0 z-50 w-full h-16 bg-white border-t border-gray-200 dark:bg-gray-700 dark:border-gray-600"> -->
    <!-- <div class="grid max-w-lg grid-cols-4 mx-auto font-medium h-16 bottom-0">
